...
export default function CandidateCard({ candidate, onViewDetails }) {
  ...
  return (
    ...
    >
      <div
        className="card-img-top-wrapper bg-body-tertiary text-center rounded-3"
      ...
      >
        {photoUrl ? (
          ...
          <div className="d-flex align-items-center justify-content-center h-100">
            <i className="bi bi-person-bounding-box display-1 text-body-secondary opacity-50"></i>
          </div>
        )}
      </div>
      <div className="card-body d-flex flex-column">
        <h5 className="card-title h6 mb-0 text-body-secondary">{displayName}</h5>
        <hr className="border-1 border-secondary opacity-20 my-2" />

        <p className="card-text text-body-secondary fs-7 small mb-1">
          <i className="bi bi-person-fill fs-7 text-body-secondary me-2"></i>{" "}
          {runningFor}
        </p>
        <p className="card-text text-body-secondary fs-7 small mb-1">
          <i className="bi bi bi-people-fill fs-7 text-body-secondary me-2"></i>{" "}
          {affiliation}
        </p>
        <div className="text-center">
          <i className="bi bi-dot fs-7 text-body-secondary"></i>
        </div>
        <p className="card-text small text-body-secondary mb-3 flex-grow-1">
          {truncatedBio}
        </p>
        <button
          onClick={() => onViewDetails(candidate)}
          className="btn custom-btn fs-7 btn-sm text-body-secondary bg-secondary-subtle" // mt-auto pushes button to bottom if card body is flex-column
        >
          View Details
        </button>
      </div>
