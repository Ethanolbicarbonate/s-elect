...
export default function ReviewBallotSection({
  ...
  const renderSelectionsForScope = (scopeSelections, scopePositions, scopeCandidates, scopeTitle) => {
    ...
    const sortedScopePositions = [...scopePositions].sort((a, b) => a.order - b.order);

    return (
      <>
        {sortedScopePositions.map(position => {
          const selectedCandidateIds = Array.from(scopeSelections[position.id] || new Set());
          let selectionDisplay;

          if (selectedCandidateIds.length === 0) {
            selectionDisplay = <span className="text-muted fst-italic">No selection made</span>;
          } else {
            selectionDisplay = (
              <ul className="list-unstyled mb-0 ps-3">
                {selectedCandidateIds.map(candId => (
                  <li key={candId} className="fw-medium text-dark-emphasis">
                    {findNameById(candId, scopeCandidates, 'firstName')} {findNameById(candId, scopeCandidates, 'lastName')}
                  </li>
                ))}
              </ul>
            );
          ...
          return (
            <div key={position.id} className="mb-3 pb-3 border-bottom">
              <h6 className="fw-semibold text-secondary mb-1">{position.name}</h6>
              {selectionDisplay}
            </div>
          );
        })}
  ...
  const totalUscSelections = Object.values(uscSelections).reduce((sum, set) => sum + set.size, 0);
  const totalCscSelections = Object.values(cscSelections).reduce((sum, set) => sum + set.size, 0);

  const handleDownloadSelections = () => {
    let content = `My Ballot Selections for ${electionData?.name || 'Election'}\n========================================\n\n`;
    content += `UNIVERSITY STUDENT COUNCIL (USC)\n------------------------------\n`;
    (electionData?.uscPositions || []).sort((a,b)=>a.order - b.order).forEach(pos => {
        content += `${pos.name}:\n`;
        const selectedIds = Array.from(uscSelections[pos.id] || new Set());
        if (selectedIds.length > 0) {
            selectedIds.forEach(id => content += `  - ${findNameById(id, electionData.uscCandidates, 'firstName')} ${findNameById(id, electionData.uscCandidates, 'lastName')}\n`);
        } else {
    ...
    content += `\nCOLLEGE STUDENT COUNCIL (${studentCollegeName || 'CSC'})\n------------------------------\n`;
     (electionData?.cscPositions || []).sort((a,b)=>a.order - b.order).forEach(pos => {
        content += `${pos.name}:\n`;
        const selectedIds = Array.from(cscSelections[pos.id] || new Set());
        if (selectedIds.length > 0) {
            selectedIds.forEach(id => content += `  - ${findNameById(id, electionData.cscCandidates, 'firstName')} ${findNameById(id, electionData.cscCandidates, 'lastName')}\n`);
        } else {
            content += `  (No selection)\n`;
        }
        content += "\n";
    ...
    link.download = `my_ballot_selections_${electionData?.id || 'election'}.txt`;
    ...
    <div className="review-ballot-section my-4">
      <div className="text-center mb-4">
        ...
        <p className="lead text-muted">
          Please carefully review your selections below. You can edit your choices before submitting.
        </p>
      </div>
      ...
      <div className="card mb-4 shadow-sm">
        <div className="card-header bg-light d-flex justify-content-between align-items-center">
          <h4 className="h5 mb-0 text-dark-emphasis">University Student Council (USC)</h4>
          <button className="btn btn-sm btn-outline-secondary" onClick={onEditUSC}>
            <i className="bi bi-pencil-square me-1"></i> Edit USC
          </button>
        </div>
        <div className="card-body p-4">
          {renderSelectionsForScope(uscSelections, electionData?.uscPositions, electionData?.uscCandidates, "USC")}
        </div>
      </div>
      ...
      {electionData?.cscPositions && electionData.cscPositions.length > 0 && (
        <div className="card mb-4 shadow-sm">
          <div className="card-header bg-light d-flex justify-content-between align-items-center">
            <h4 className="h5 mb-0 text-dark-emphasis">College Student Council ({studentCollegeName || 'CSC'})</h4>
            <button className="btn btn-sm btn-outline-secondary" onClick={onEditCSC}>
              <i className="bi bi-pencil-square me-1"></i> Edit CSC
            </button>
          </div>
          <div className="card-body p-4">
            {renderSelectionsForScope(cscSelections, electionData?.cscPositions, electionData?.cscCandidates, `CSC - ${studentCollegeName}`)}
          </div>
        </div>
      )}
      ...
      {(totalUscSelections === 0 && totalCscSelections === 0) && (
          <div className="alert alert-warning text-center">
              You have not made any selections. Are you sure you want to proceed with an empty ballot?
          </div>
      )}
      ...
      <div className="mt-4 pt-4 border-top d-flex flex-column flex-sm-row justify-content-between align-items-center gap-2">
        ...
        >
          Confirm and Cast My Vote <i className="bi bi-send-check-fill ms-2"></i>
        </button>
      </div>
    </div>
  );
