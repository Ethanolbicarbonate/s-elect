// app/(public)/student-signup/page.js
'use client';
...
export default function StudentSignupForm() {
  ...
  const handleSubmitEmail = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Failed to initiate sign-up.');
      } else {
        setMessage(data.message);
        // Redirect to the verification/password setup page, passing email
        router.push(`/student-signup/verify?email=${encodeURIComponent(email)}`);
      }
    } catch (err) {
      console.error(err);
  ...
  return (
      <div className="flex-grow-1 d-flex flex-column align-items-center px-4 overflow-auto">
        ...
        <form onSubmit={handleSubmitEmail} className="w-100" style={{ maxWidth: '400px' }}>
