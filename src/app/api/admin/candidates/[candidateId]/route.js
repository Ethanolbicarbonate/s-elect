...
export async function DELETE(request, context) {
  ...
  try {
    const candidateToDelete = await prisma.candidate.findUnique({
        where: { id: candidateId },
        include: { position: true } // Keep this for moderator check
    });
    ...
    // Moderator Authorization (keep your existing logic here)
    if (session.user.role === 'MODERATOR') {
        // ... your moderator scope checks ...
    ...
    // --- MODIFIED PART ---
    // The `onDelete: Restrict` in VoteCast for candidateId will prevent deletion if votes exist.
    // Prisma will throw a P2003 error in that case.
    ...
    if (error.code === 'P2003') { // Foreign key constraint violation
        // This means there are VoteCast records pointing to this candidate.
        return NextResponse.json(
            { error: 'Cannot delete this candidate as they have received votes. Consider an alternative action like disqualification if needed.' },
